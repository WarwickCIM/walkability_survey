...
df <- data_preparations("data/raw/survey_platial_demo.csv") %>% 
...
p <- ggplot(ratings_avg, aes(x = answer, y = n, fill = age)) +
  ...
  facet_grid(vars(question)) +
  scale_fill_brewer(palette = "Pastel1") +
  labs(title = "Average ratings by question and age",
       x = NULL, y = NULL) +
  theme_minimal() +
  theme(
    axis.line.x = element_blank(),
...
p <- ggplot(ratings_avg, aes(x = answer, y = n, fill = ethnicity)) +
  ...
  facet_grid(vars(question)) +
  scale_fill_brewer(palette = "Pastel1") +
  labs(title = "Average ratings by question and ethnicity",
       x = NULL, y = NULL) +
  theme_minimal() +
  theme(
    axis.line.x = element_blank(),
...
p <- df %>% 
  ...
  geom_tile() + 
  labs(title ="Objects' influence on walkability", 
       x = "", y = "") +
