...
#' @param df Dataframe with survey's responses, after being cleaned by
#'   data_preparations().
#' @return A long dataframe with image ratings and responses_id, ready to
...
get_image_ratings <- function(df) {
  ...
  # Iterates over cluster and images to get every image filename and rating.
  # There are four clusters. Each cluster has 3 sets of images that are
  # displayed from 1:33, 34:66 and 67:99, respectively.
  for (clstr in 0:4) {
    for (img in 1:3) {
      ratings_df_tmp <- df %>%
        mutate(cluster = paste0("C", clstr)) %>%
        select(response_id, cluster, paste0("img_c", clstr, "_", img),
               starts_with(c(
          paste0("enjoyment_img_c", clstr, "_", img),
          paste0("aesthetics_img_c", clstr, "_", img),
          paste0("safety_img_c", clstr, "_", img),
          paste0("vibrancy_img_c", clstr, "_", img)
        ))) %>%
        # Creates a column, image_num,with the right filename, resulting from
        # combining cluster and image iterations.
        unite(
          image_num,
          cluster, paste0("img_c", clstr, "_", img)
        ) %>%
  ...
  ratings_df <- ratings_df %>%
    ...
    mutate(
      cluster = as.factor(str_sub(image_num, 1, 2)),
      rating_n = as.numeric(rating)
    )

  ratings_df_extended <- df %>%
    select(
      response_id, gender, age, ethnicity,
...
get_ratings_table <- function(df, rating_category, img_width = 160) {
  df %>%
    ...
    mutate(img = paste0(
      "<a href='https://raw.githubusercontent.com/WarwickCIM/walkability_survey/gsv_images/img/", image_num, "_composite.jpg'>",
